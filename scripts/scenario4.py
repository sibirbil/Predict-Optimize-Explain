...
# EXPERIMENT 3 
# HOW DO SUMMERCHILD AND WINTERFOLF BEHAVE DIFFERENTLY?
# we generate scenarios where winterwolf outperforms summerchild (in either return or sharpe)
# and vice versa. We will first generate scenarios where the outputs are distinct returns
# and then analyze the data splitting it into two groups where one of them outperforms the other.
ASSET_SIZE = 30
DATE = 202001
LAMBDA = 10.
KAPPA = 1.0
...
def main(    
    ...
    for i in range(n_seeds):
        if lasts is None:
            m_start = m0 + (b - a)*0.05*torch.rand((9,))
        else:
            m_start = lasts[i]
