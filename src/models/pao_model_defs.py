...
Contents:
  - AssetPricingFNN + load_fnn_from_dir (for selection)
...
# =============================================================================
# 1) PTO-style FNN definition + loader
# =============================================================================
...
class DifferentiableMVOLayer(nn.Module):
    """
    max mu'w - (gamma/2) w'Σw  s.t. sum(w)=1, w>=0
    ...
    def __init__(self, n_assets: int, lambda_: float):
        ...
        obj = cp.Minimize(-mu @ w + (float(lambda_) / 2.0) * risk)
        cons = [cp.sum(w) == 1]
...
class DifferentiableRobustMVOLayer(nn.Module):
    """
    max mu'w - kappa||A w||_2 - (gamma/2) w'Σw  s.t. sum(w)=1, w>=0
    """
    def __init__(self, n_assets: int, lambda_: float, kappa: float):
        ...
        risk = cp.sum_squares(U @ w)
        obj = cp.Minimize(-mu @ w + float(kappa) * cp.norm(A @ w, 2) + (float(lambda_) / 2.0) * risk)
        cons = [cp.sum(w) == 1]
        prob = cp.Problem(obj, cons)
...
class PAOPortfolioModel(nn.Module):
    def __init__(
        ...
        if self.kappa > 0:
            self.opt = DifferentiableRobustMVOLayer(self.n_assets, self.lambda_, self.kappa)
        else:
            self.opt = DifferentiableMVOLayer(self.n_assets, self.lambda_)
...
def load_pao_model_from_run(
    ...
    # Support both "gamma" (old) and "lambda_" (new) keys for backward compatibility
    lambda_val = float(cfg.get("lambda_", cfg.get("gamma", 5.0)))
